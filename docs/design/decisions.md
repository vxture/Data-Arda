...
pro < business < enterprise`. The `tierMeets(tier, min)` utility compares by rank
index. Each tier is an independently-maintained product (its own features and
limits), not a single shared flag.
