...
#
# Two-host topology: this stack is plain HTTP on the deploy host's tailnet, so the app
# is verified directly on its published port. TLS / the public domain / the
# OIDC login surface are fronted by the shared public edge and are verified
# there, not here.
set -euo pipefail
...
# TLS is terminated at the edge; on the deploy host the app speaks plain HTTP. Probe
